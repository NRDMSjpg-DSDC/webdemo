...
const homeCenter = fromLonLat([88.75, 26.65]);

const styles = {
  riskHigh: new Style({
    fill: new Fill({ color: "rgba(210,55,35,.28)" }),
    stroke: new Stroke({ color: "#b52d1f", width: 2 }),
  }),
  riskMed: new Style({
    fill: new Fill({ color: "rgba(240,173,78,.28)" }),
    stroke: new Stroke({ color: "#b87818", width: 2 }),
  }),
  road: new Style({ stroke: new Stroke({ color: "#666", width: 2 }) }),
  blocked: new Style({
    stroke: new Stroke({ color: "#d32f2f", width: 4, lineDash: [10, 8] }),
  }),
  route: new Style({ stroke: new Stroke({ color: "#16a34a", width: 5 }) }),
  shelter: new Style({
    image: new Circle({
      radius: 8,
      fill: new Fill({ color: "#1769aa" }),
...
const riskLayer = vectorLayer("data/risk_zones.geojson", (f) =>
  f.get("risk") === "High" ? styles.riskHigh : styles.riskMed,
);

const sheltersLayer = vectorLayer("data/shelters.geojson", styles.shelter);

const roadsLayer = vectorLayer("data/roads.geojson", (f) =>
  f.get("status") === "Blocked" ? styles.blocked : styles.road,
);
...
const gramPanchayatLayer = vectorLayer(
  "data/gram_panchayat.geojson",
  (f) =>
    new Style({
      fill: new Fill({ color: "rgba(142,68,173,.10)" }),
      stroke: new Stroke({ color: "#7b1fa2", width: 1, lineDash: [8, 6] }),
      text: new Text({
        text: f.get("Gram_Panchayat") || f.get("name") || "",
        font: "bold 11px sans-serif",
        fill: new Fill({ color: "#5e2a72" }),
...
document.getElementById("popupCloser").onclick = () =>
...
document.querySelectorAll("[data-layer]").forEach((cb) => {
  cb.addEventListener("change", () => {
    const layers = {
      ...
      gramPanchayat: gramPanchayatLayer,
    };
...
document.getElementById("locateBtn").onclick = () => {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (err) => {
      ...
      if (err.code === 1)
        message =
          "Location permission was denied. Please allow Location access for localhost:8000 and try again.";
      else if (err.code === 2)
        message =
          "Your location could not be determined. Check GPS/Wi-Fi/location services and try again.";
      else if (err.code === 3)
...
function zoomFeatures(fs) {
  ...
  map
    .getView()
    .fit(ex, { padding: [90, 90, 90, 90], maxZoom: 14, duration: 600 });
}
function setupGPSelectors() {
  gpFeatures = gramPanchayatLayer.getSource().getFeatures();

  if (!gpFeatures.length) {
    setTimeout(setupGPSelectors, 250);
    return;
  }
  const blocks = [
    ...new Set(gpFeatures.map((f) => f.get("Block")).filter(Boolean)),
  ].sort();

  const bs = document.getElementById("blockSelect"),
    gs = document.getElementById("gpSelect");

  const db = document.getElementById("downloadBlock"),
    dg = document.getElementById("downloadGP");
  ...
  function fill(s, b) {
    ...
    gpFeatures
      ...
      .forEach((f) => {
        let o = document.createElement("option");
        o.value = f.get("name");
        o.textContent = f.get("name");
        s.appendChild(o);
      });
  }
  bs.onchange = () => {
    fill(gs, bs.value);
    let fs = gpFeatures.filter((f) => f.get("Block") === bs.value);
    if (fs.length) zoomFeatures(fs);
  };

  gs.onchange = () => {
    let f = gpFeatures.find((f) => f.get("name") === gs.value);
    if (f) zoomFeatures([f]);
  };
...
document.getElementById("downloadClose").onclick = () =>
...
document.getElementById("pdfDownloadBtn").onclick = () => {
  const b = document.getElementById("downloadBlock").value,
    g = document.getElementById("downloadGP").value,
    m = document.getElementById("downloadMessage");

  if (!b || !g) {
    m.textContent = "Please select both Block and Gram Panchayat.";
    return;
  }
  const file =
    "pdf/" +
    b.replace(/[^a-z0-9]+/gi, "_") +
    "__" +
...
document.getElementById("analyzeBtn").onclick = () => {
  ...
  box.innerHTML =
    plans[v] || "Select an affected area to calculate the evacuation plan.";
};
...
document.querySelectorAll(".emergency-menu-item").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll(".emergency-menu-item")
    ...
    if (key === "facilities" && facilitiesLayer)
      facilitiesLayer.setVisible(true);
...
function statFeatures(layer) {
  return layer && layer.getSource ? layer.getSource().getFeatures() : [];
}
...
function putStat(id, v) {
  ...
  if (e) e.textContent = Number(v || 0).toLocaleString("en-IN");
}
function updateEmergencyStats() {
  const gp = statFeatures(gramPanchayatLayer),
    sh = statFeatures(sheltersLayer),
    ef = statFeatures(facilitiesLayer);
...
(function waitEmergencyStats() {
  if (
    statFeatures(gramPanchayatLayer).length ||
    statFeatures(sheltersLayer).length ||
    statFeatures(facilitiesLayer).length
  )
...
(function () {
  ...
  function setCollapsed(collapsed) {
    ...
    toggle.setAttribute(
      "aria-label",
      collapsed ? "Open left menu" : "Collapse left menu",
    );
...
(function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    const u = document.getElementById("loginUsername").value.trim(),
      ...
      t = document.getElementById("loginUserType").value;

    const a = accounts.find(
      (x) => x.username === u && x.password === p && x.type === t,
    );
    ...
    if (a) {
      ...
      sessionStorage.setItem("dmwgisLoggedIn", "1");
      sessionStorage.setItem("dmwgisUser", a.username);
      sessionStorage.setItem("dmwgisRole", a.type);
